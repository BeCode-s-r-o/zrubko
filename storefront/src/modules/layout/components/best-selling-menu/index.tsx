...
export default function BestSellingMenu() {
  ...
  useEffect(() => {
    async function loadBestSellingCategories() {
      try {
        ...
        // Nájdeme parent kategóriu "Najpredávanejšie produkty" podľa handle
        const bestSellingParent = product_categories?.find(
          (cat) => cat.handle === 'najpredavanejsie-produkty' && !cat.parent_category_id
        )
        
        if (bestSellingParent) {
          // Filtrujeme len kategórie, ktoré majú parent_category_id = Najpredávanejšie produkty
          const bestSellingCategories = product_categories?.filter(
            (cat) => cat.parent_category_id === bestSellingParent.id
          ) || []
          setCategories(bestSellingCategories)
        } else {
          setCategories([])
        }
      } catch (error) {
  ...
  // Mapovanie kategórií na ikony (rovnaké ako v CategoryMenu a OsmoMenu)
  ...
  return (
    <li className="nav-item dropdown-item-pages dropdown py-2 py-xl-5 px-0 px-xl-4">
      <a href={`/${countryCode}/store`} className="nav-link dropdown-toggle p-0" id="bestSellingDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" onClick={(e) => e.preventDefault()}>
        Najpredávanejšie produkty <span className="caret"></span>
      </a>
      <div className="dropdown-menu py-0" aria-labelledby="bestSellingDropdown" style={{ minWidth: '250px' }}>
        {loading ? (
